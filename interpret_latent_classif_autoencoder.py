...
def get_latent_masks(model: ClassifierAutoEncoder,
                     masked_neighbor_image: torch.Tensor) -> torch.Tensor:
...
def get_sd_map(img_neighbor: torch.Tensor,
    ...
    latent_masks = get_latent_masks(model, masked_image)
...
def main(config: dict):
    ...
    # Use the train set to analyze the decisions over the trained data
    ...
    # Trained autoencoder
    model = ClassifierAutoEncoder(input_image_size)
    ...
    for idx in query_idx:
        ...
        sim_agg = torch.zeros(input_image_size)
        dis_agg = torch.zeros(input_image_size)
